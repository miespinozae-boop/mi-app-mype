...
.welcome-title{font-size:30px;font-weight:800;color:#172B4D!important}
...
if page=="Inicio":
    ...
    with x:
        ...
        fig=px.bar(z,x="Ventas",y="Producto",orientation="h",text_auto=".0f"); fig.update_traces(marker_color=["#4B9AF5","#20B486","#8B6FE8","#F6A23A","#27B8C7"]*3); fig.update_layout(height=430,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); st.plotly_chart(fig,use_container_width=True)
    with y:
        st.subheader("Top productos por rentabilidad"); z=prod.nlargest(10,"Margen_%").sort_values("Margen_%")
        fig=px.bar(z,x="Margen_%",y="Producto",orientation="h",text_auto=".1f"); fig.update_traces(marker_color=["#8B6FE8","#20B486","#4B9AF5","#F6A23A","#27B8C7"]*3); fig.update_layout(height=430,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); st.plotly_chart(fig,use_container_width=True)
    ...
    x,y=st.columns(2)
    with x:
        st.subheader("Utilidad por categoría"); fig=px.bar(cat.sort_values("Utilidad"),x="Utilidad",y="Categoria",orientation="h",text_auto=".0f"); fig.update_traces(marker_color=["#20B486","#8B6FE8","#4B9AF5","#F6A23A","#27B8C7"]); fig.update_layout(height=380,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); st.plotly_chart(fig,use_container_width=True)
    with y:
        st.subheader("Margen por categoría"); fig=px.bar(cat.sort_values("Margen_%"),x="Margen_%",y="Categoria",orientation="h",text_auto=".1f"); fig.update_traces(marker_color=["#4B9AF5","#20B486","#8B6FE8","#F6A23A","#27B8C7"]); fig.update_layout(height=380,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); st.plotly_chart(fig,use_container_width=True)
    ...
    if f.empty: st.warning("No hay datos para simular.")
        ...
        fig.update_layout(barmode="group",height=330,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); st.plotly_chart(fig,use_container_width=True)
