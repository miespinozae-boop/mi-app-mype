...
.block-container{padding-top:4.5rem!important;max-width:1500px!important}
...
.welcome-title{font-size:30px!important;font-weight:800!important;color:#172B4D!important;line-height:1.35!important;margin-top:.25rem!important;overflow:visible!important}
...
section[data-testid="stFileUploaderDropzone"]{background:#FFFFFF!important;border:1px solid #DCE5F0!important;border-radius:10px!important}\n[data-testid="stFileUploader"] small{color:#60738F!important}\n.sidebar-note{border:1px solid #DCE5F0;border-radius:12px;padding:14px;margin-top:18px;background:#fff;color:#60738F!important;font-size:12px}
...
def render_chart(fig, height=None):
    # Streamlit puede aplicar un tema que vuelva blancos los textos de Plotly.
    # Forzamos una apariencia clara y legible en todos los gráficos.
    layout = dict(
        template="plotly_white",
        paper_bgcolor="white",
        plot_bgcolor="white",
        font=dict(color="#172B4D", family="Arial"),
        legend=dict(font=dict(color="#334A68")),
        hoverlabel=dict(bgcolor="white", font=dict(color="#172B4D")),
        xaxis=dict(
            tickfont=dict(color="#60738F"),
            titlefont=dict(color="#536A87"),
            gridcolor="#E8EEF5",
            zerolinecolor="#DCE5F0"
        ),
        yaxis=dict(
            tickfont=dict(color="#60738F"),
            titlefont=dict(color="#536A87"),
...
with st.sidebar:
    ...
    up=st.file_uploader("Excel o CSV",type=["xlsx","xls","csv"],key="upload")
...
customers=int(f.Cliente.nunique()) if "Cliente" in f.columns else int(max(0,units*.72))
...
if page=="Inicio":
    ...
    with l:
        ...
        if not daily.empty:
            ...
            fig.update_layout(height=300,margin=dict(l=10,r=10,t=10,b=10),plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D"),xaxis=dict(gridcolor="#EDF2F7"),yaxis=dict(gridcolor="#EDF2F7"))
            render_chart(fig)
    ...
    with r:
        ...
        if not cat.empty and cat.Ventas_Soles.sum()>0:
            ...
            fig.update_layout(height=300,margin=dict(l=0,r=0,t=0,b=0),paper_bgcolor="white",font=dict(color="#172B4D"))
            render_chart(fig)
    ...
    with p1:
        ...
        if not prod.empty:
            ...
            fig.update_layout(height=270,margin=dict(l=0,r=10,t=10,b=10),plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D"),showlegend=False)
            render_chart(fig)
    with p2:
    ...
    daily=f.groupby("Fecha",as_index=False).Ventas_Soles.sum()
    fig=px.line(daily,x="Fecha",y="Ventas_Soles",markers=True); fig.update_traces(line_color="#4B9AF5",line_width=3); fig.update_layout(height=420,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D"))
    render_chart(fig)
    ...
    with x:
        ...
        fig=px.bar(z,x="Ventas",y="Producto",orientation="h",text_auto=".0f"); fig.update_traces(marker_color=["#4B9AF5","#20B486","#8B6FE8","#F6A23A","#27B8C7"]*3); fig.update_layout(height=430,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); render_chart(fig)
    with y:
        st.subheader("Top productos por rentabilidad"); z=prod.nlargest(10,"Margen_%").sort_values("Margen_%")
        fig=px.bar(z,x="Margen_%",y="Producto",orientation="h",text_auto=".1f"); fig.update_traces(marker_color=["#8B6FE8","#20B486","#4B9AF5","#F6A23A","#27B8C7"]*3); fig.update_layout(height=430,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); render_chart(fig)
    ...
    x,y=st.columns(2)
    with x:
        st.subheader("Utilidad por categoría"); fig=px.bar(cat.sort_values("Utilidad"),x="Utilidad",y="Categoria",orientation="h",text_auto=".0f"); fig.update_traces(marker_color=["#20B486","#8B6FE8","#4B9AF5","#F6A23A","#27B8C7"]); fig.update_layout(height=380,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); render_chart(fig)
    with y:
        st.subheader("Margen por categoría"); fig=px.bar(cat.sort_values("Margen_%"),x="Margen_%",y="Categoria",orientation="h",text_auto=".1f"); fig.update_traces(marker_color=["#4B9AF5","#20B486","#8B6FE8","#F6A23A","#27B8C7"]); fig.update_layout(height=380,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); render_chart(fig)
    ...
    if f.empty: st.warning("No hay datos para simular.")
        ...
        fig.update_layout(barmode="group",height=330,plot_bgcolor="white",paper_bgcolor="white",font=dict(color="#172B4D")); render_chart(fig)
